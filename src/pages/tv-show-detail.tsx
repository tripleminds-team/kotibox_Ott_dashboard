  ...
  return (
    <div className="min-h-screen bg-[#030306] text-white">
      ...
      <div className="relative w-full" style={{ height: "70vh", minHeight: 440 }}>
        ...
        <button
          onClick={() => setLocation("/")}
          className="absolute top-20 left-6 sm:left-10 flex items-center gap-2 text-zinc-200 hover:text-white text-sm font-semibold transition-colors z-10"
        >
        ...
        <div className="absolute bottom-10 left-0 px-6 sm:px-10 lg:px-14 max-w-2xl">
          ...
          <div className="flex items-center gap-3 mb-3 flex-wrap text-sm">
            ...
            {year && <span className="text-zinc-200 font-semibold">{year}</span>}
            <span className="text-zinc-200 font-semibold">{seasons} Season{seasons !== 1 ? "s" : ""}</span>
            <span className="text-zinc-200 font-semibold">{totalEps} Episodes</span>
          </div>

          <p className="text-zinc-100 text-sm leading-relaxed mb-6 max-w-lg line-clamp-3">{description}</p>
      ...
      <div className="px-6 sm:px-10 lg:px-14 pb-20 mt-6">
        ...
        <div className="mb-12">
              ...
              <div className="flex items-center gap-2">
                {uniqueSeasons.map((s) => (
                  <button
                    ...
                    className={`px-4 py-1.5 rounded-full text-sm font-bold border transition-all ${selectedSeason === s ? "bg-red-600 border-red-600 text-white" : "bg-transparent border-zinc-700 text-zinc-200 hover:border-zinc-500 hover:text-white"}`}
                  >
                    S{s}
                  </button>
                ))}
          ...
          {seasonEpisodes.length === 0 ? (
            <div className="flex items-center justify-center py-16 text-zinc-100 text-sm">
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0 py-1">
                      ...
                      {ep.description && (
                        <p className="text-zinc-100 text-xs mt-1 line-clamp-2 leading-relaxed">{ep.description}</p>
                      )}
                      <button
                        onClick={(e) => { e.stopPropagation(); handleDownloadEpisode(ep); }}
                        disabled={downloadingEp === (ep._id || ep.id)}
                        className="mt-2 flex items-center gap-1 text-[10px] font-bold text-zinc-100 hover:text-emerald-400 transition-colors disabled:opacity-50"
                      >
        ...
        <div className="mb-12 max-w-3xl">
          ...
          <p className="text-zinc-100 text-sm leading-relaxed">{description || "No description available."}</p>
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mt-6">
            {year && (
              <div>
                <p className="text-zinc-100 text-xs font-bold uppercase tracking-wider mb-1">Year</p>
                <p className="text-white text-sm font-semibold">{year}</p>
              </div>
            )}
            {genres.length > 0 && (
              <div>
                <p className="text-zinc-100 text-xs font-bold uppercase tracking-wider mb-1">Genres</p>
                <p className="text-white text-sm font-semibold">{genres.slice(0, 3).join(", ")}</p>
              </div>
            )}
            {totalEps > 0 && (
              <div>
                <p className="text-zinc-100 text-xs font-bold uppercase tracking-wider mb-1">Episodes</p>
                <p className="text-white text-sm font-semibold">{totalEps} Episodes</p>
              </div>
            )}
            {imdbRating && (
              <div>
                <p className="text-zinc-100 text-xs font-bold uppercase tracking-wider mb-1">Rating</p>
                  ...
                  <div key={i} className="flex-shrink-0 text-center w-20">
                    <div className="w-16 h-16 rounded-full bg-zinc-800 overflow-hidden mx-auto mb-2">
                      {actorImg ? (
                        ...
                        <div className="w-full h-full flex items-center justify-center text-zinc-200 text-lg font-bold">
                          {actorName?.[0] || "?"}
                        </div>
                      )}
                    </div>
                    <p className="text-white text-[11px] font-semibold leading-tight line-clamp-2">{actorName}</p>
                    {c.character && <p className="text-zinc-100 text-[10px] mt-0.5 truncate">{c.character}</p>}
                  </div>
                );
              })}
            </div>
