...
import { ImageIcon, Plus, X, Trash2, Sparkles, Upload as UploadIcon } from "lucide-react";
...
type Tab = "TV Show" | "Basic Info" | "SEO Settings" | "Clip Details";
const TABS: Tab[] = ["TV Show", "Basic Info", "SEO Settings", "Clip Details"];
...
export default function TvShowForm() {
  ...
  const [trailerUrlType, setTrailerUrlType] = useState("select");
  const [trailerUrl, setTrailerUrl] = useState("");
  const [trailerFile, setTrailerFile] = useState<File | null>(null);
  const [description, setDescription] = useState("");
  const [access, setAccess] = useState<"paid" | "free">("paid");
    ...
    <div className="space-y-5 pb-6">
      ...
      <div className="rounded-xl border border-border bg-card p-4">
        <p className="text-sm text-zinc-400 mb-3 flex items-center gap-1.5">
          ...
          Import TV Show From TMDB ( Add the TV Show ID)
        </p>
        <div className="flex gap-3">
          <Input
            placeholder="e.g. #TV12345"
            value={tmdbId}
      ...
      <div className="rounded-xl border border-border bg-card">
        ...
        {activeTab === "TV Show" && (
          <div className="p-6 space-y-6">
            <SectionHeading title="About TV Show" />

            <div className="rounded-xl border border-border bg-muted/10 p-5">
              <div className="flex gap-5">
                <ImageBox label="Thumbnail" preview={thumbnail.preview} onChange={(f, url) => setThumbnail({ file: f, preview: url })} />
                <ImageBox label="Poster" preview={poster.preview} onChange={(f, url) => setPoster({ file: f, preview: url })} />
                <ImageBox label="Poster Tv Image" preview={posterTv.preview} onChange={(f, url) => setPosterTv({ file: f, preview: url })} />
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
              ...
              <div className="space-y-1.5">
                <Label className="text-foreground text-sm font-medium">Trailer URL Type <span className="text-red-500">*</span></Label>
            ...
            {trailerUrlType === "local" && (
              <div className="space-y-1.5">
                ...
                />
                {trailerFile && <p className="text-xs text-zinc-500 truncate">{trailerFile.name}</p>}
              </div>
            )}

            {trailerUrlType !== "local" && trailerUrlType !== "select" && (
              <div className="space-y-1.5">
                <Label className="text-foreground text-sm font-medium">Trailer URL <span className="text-red-500">*</span></Label>
                <Input
            ...
            <div className="space-y-1.5">
              <div className="flex items-center justify-between">
                <Label className="text-foreground text-sm font-medium">Description <span className="text-red-500">*</span></Label>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-5 items-start">
              ...
              <div className="space-y-1.5">
                <Label className="text-foreground text-sm font-medium">Plan <span className="text-red-500">*</span></Label>
          ...
          <div className="p-6 space-y-6">
            ...
            <div className="rounded-xl border border-border bg-muted/10 p-5 space-y-5">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
                <div className="space-y-1.5">
                  <Label className="text-foreground text-sm font-medium">Language <span className="text-red-500">*</span></Label>
                ...
                <div className="space-y-1.5">
                  <Label className="text-foreground text-sm font-medium">Genres <span className="text-red-500">*</span></Label>
                  <Input
                    placeholder="Genres"
                    value={genres}
              ...
              <div className="flex items-center justify-between p-4 rounded-xl border border-border bg-card">
                ...
                <Switch checked={ageRestricted} onCheckedChange={setAgeRestricted} className="data-[state=checked]:bg-red-600" />
              </div>
            </div>
            ...
            <div className="rounded-xl border border-border bg-muted/10 p-5">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                <div className="space-y-1.5">
                  <Label className="text-foreground text-sm font-medium">Actors <span className="text-red-500">*</span></Label>
                  <Textarea
                    placeholder="Actors"
                    value={actors}
                ...
                <div className="space-y-1.5">
                  <Label className="text-foreground text-sm font-medium">Directors <span className="text-red-500">*</span></Label>
                  <Textarea
                    ...
                    <div className="md:col-span-2 space-y-5">
                      ...
                      <div className="space-y-1.5">
                        ...
                        <div
                          ...
                          onClick={() => document.getElementById("tv-kw-input")?.focus()}
                        >
                          {keywords.map((kw) => (
                            <span key={kw} className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full bg-red-600/20 text-red-300 text-xs font-medium">
                              {kw}
                              <button type="button" onClick={(e) => { e.stopPropagation(); setKeywords((p) => p.filter((k) => k !== kw)); }}>
                                <X className="h-3 w-3" />
                              </button>
                            </span>
                          ))}
        ...
        {activeTab === "Clip Details" && (
          <div className="p-6 space-y-4">
            <div className="flex items-center justify-between">
              ...
              <Button type="button" onClick={addClipRow} className="bg-red-600 hover:bg-red-700 text-white h-9 gap-2 rounded-lg px-4 text-sm font-semibold">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 items-end">
                      <div className="space-y-1.5">
                        ...
                        <Input
                          ...
                          value={row.name}
                          onChange={(e) => setClipRows((p) => p.map((r) => r.id === row.id ? { ...r, name: e.target.value } : r))}
                          className="bg-muted border-border text-foreground placeholder:text-muted-foreground h-10 rounded-lg text-sm"
                        />
                      </div>
                      <div className="space-y-1.5">
                        ...
                        <Select
                          value={row.type}
                          onValueChange={(v) => setClipRows((p) => p.map((r) => r.id === row.id ? { ...r, type: v } : r))}
                        >
                          <SelectTrigger className="bg-muted border-border text-foreground h-10 rounded-lg text-sm">
                            <SelectValue />
                          </SelectTrigger>
                      ...
                      <div className="flex gap-2 items-end">
                        <div className="flex-1 space-y-1.5">
                          <Label className="text-foreground text-sm font-medium">{row.type === "local" ? "Clip File" : "Clip URL"}</Label>
                          {row.type === "local" ? (
                            <Input
                              type="file" accept="video/*"
                              onChange={(e) => setClipRows((p) => p.map((r) => r.id === row.id ? { ...r, file: e.target.files?.[0] ?? null } : r))}
                              className="bg-muted border-border text-foreground h-10 rounded-lg text-sm file:mr-3 file:text-muted-foreground file:bg-muted file:border-0 file:text-sm cursor-pointer"
                            />
                          ) : (
                            <Input
                              placeholder="https://..."
                              value={row.url}
                              onChange={(e) => setClipRows((p) => p.map((r) => r.id === row.id ? { ...r, url: e.target.value } : r))}
                              className="bg-muted border-border text-foreground placeholder:text-muted-foreground h-10 rounded-lg text-sm"
                            />
                          )}
                        </div>
                        <button
                          type="button" onClick={() => removeClipRow(row.id)}
      ...
      <div className="flex justify-end">
        <Button onClick={handleSave} className="bg-red-600 hover:bg-red-700 text-white h-11 px-10 rounded-lg font-semibold text-sm">
          Save
        </Button>
      </div>
    </div>
